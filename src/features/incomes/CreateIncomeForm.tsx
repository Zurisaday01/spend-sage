...
const formSchema = z.object({
	...
	amount: z.string().regex(/^(?!0$)[0-9]+$/, 'Amount must be different than 0'),
});
...
const CreateIncomeForm = () => {
	...
	const { data: categories, isLoading: isLoadingCategories } =
		useGetCategoriesQuery({ type: 'income', userId: user?.id });
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			...
			if (result.data && 'transaction' in result.data) {
				...
				// feedback
				toast.success(
					`Income recorded: '${title}'. Your financial future thanks you for this boost! 🌟💸`
				);
			}
			if (result.error) {
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormControl>
								<Input
									placeholder='Where did your money come from?'
									{...field}
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormControl>
								<Input
									placeholder='How much did your wallet grow?'
									type='number'
									step='.01'
									min='0'
									{...field}
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormControl>
								<Textarea
									placeholder='How did you earn this money?'
									{...field}
									disabled={isLoading || form.formState.isSubmitting}
								/>
							</FormControl>
